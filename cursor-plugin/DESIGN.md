...
Preserve the compact, neutral system-tool character of CLIProxyAPI's own plugin resource examples. The memorable element is the explicit separation between Cursor dashboard subscription usage and local estimated usage: official percentages are shown as returned, and missing data stays unknown instead of becoming a fabricated remainder.
...
- Cursor dashboard usage is read with the account session cookie and `Origin: https://cursor.com`. Official `autoPercentUsed` / `apiPercentUsed` values are displayed as percentages; dollar amounts come from aggregated usage events. If those requests fail, the page shows an unavailable state with the error and never invents a remaining-quota bar.

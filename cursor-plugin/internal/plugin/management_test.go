...
func Test_Handler_ManagementResource_serves_bilingual_shell_without_exposing_auth_data(t *testing.T) {
	...
	require.Contains(t, string(response.Body), `logicalRequestsHelp: "每次用户调用最多计一个最终结果；检查点回\u2060退和宿主重试不会重复计入请求。近期请求 ID 使用固定内存去重，极低概率的哈希冲突可能使本地估算少计；Token 按插件实际执行次数估算。"`)
	require.Contains(t, string(response.Body), `logicalRequestsHelp: "At most one terminal outcome is recorded per user call; checkpoint fallbacks and host retries are not double-counted as requests. Recent request IDs use fixed-memory deduplication, so an extremely rare hash collision may conservatively undercount this local estimate. Tokens are estimated per plugin executor run."`)

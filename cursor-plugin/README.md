...
```sh
tar -xzf cursor-plugin-0.5.6-linux-amd64.tar.gz
cd cursor-plugin-0.5.6-linux-amd64
...
- v0.5.6 的会话检查点仅在插件进程内保存，并按账户、模型和会话严格隔离。仅追加式线性历史会尝试续传；分支、编辑、压缩、过期、重启或状态异常时会安全回退为完整重放。

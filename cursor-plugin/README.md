...
- 成功/失败请求来自 CLIProxyAPI 的 `request.complete` 终态回调；同一用户请求即使发生宿主重试，最多记录一次最终结果。近期请求 ID 通过双时间窗、固定内存的概率结构去重，不会因流量达到容量而重新计数；极低概率的哈希冲突只会使这项本地估算少计。宿主自身的成功/失败值按“调度尝试”单独展示。
